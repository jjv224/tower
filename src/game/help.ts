...
export function helpBuilder(level: number) {
  if (level == 1) {
    ...
    new Actor({
      appearance: new TextSprite({ center: true, face: "Arial", color: "#FFFFFF", size: 56, }, "Here is some information about all the actors in the game!"),
      rigidBody: new BoxBody({ cx: 8, cy: 1, width: .1, height: .1 }),
    });

    new Actor({
      appearance: new ImageSprite({ width: 0.75, height: 0.75, img: "player.png" }),
    ...
    let big_text = `Enjoy The Game! Make Sure To Have Fun!.`;
    new Actor({
